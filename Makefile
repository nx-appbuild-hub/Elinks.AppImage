...
all: clean
	...
	apprepo --destination=$(PWD)/build appdir boilerplate elinks elinks-data elinks-data

	echo ''		 													>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo ''		 													>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo 'if [ -z "$$@" ]' 											>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '  then' 													>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '    exec $${APPDIR}/bin/elinks "http://www.elinks.cz/"' 	>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '  else' 													>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '    exec $${APPDIR}/bin/elinks "$$@"' 					>> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && bin/appimagetool.AppImage  $(PWD)/build/Boilerplate.AppDir $(PWD)/Elinks.AppImage
